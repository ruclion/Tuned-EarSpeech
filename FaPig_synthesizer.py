...
syn_model_fpath = '/ceph/home/hujk17/Tuned-EarSpeech/synthesizer/saved_models/FaPig/FaPig_70k.pt'
...
if __name__ == '__main__':
    ...
    for i, _speaker_info in enumerate(input_meta):
        for j, text in enumerate(input_text):
            ...
            os.makedirs('log_FaPig_syn_wavs', exist_ok=True)
            _wav_pre = mel2wav(mel, wav_name_path=os.path.join('log_FaPig_syn_wavs', 'spk_' + str(i) + '_' + str(j) + '_pre.wav'))
            _wav_target = mel2wav(mel_reference, wav_name_path=os.path.join('log_FaPig_syn_wavs', 'spk_' + str(i) + '_' + str(j) + '_reference.wav'))
